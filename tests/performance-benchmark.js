...
async function runBenchmark() {
  console.log('🚀 Starting Performance Benchmark (2.75M Tasks, Org-Wide Scope, No Padding)\n');
  ...
  try {
    ...
    console.log('\n🌱 Seeding Target Data (250,000 tasks)...');
    ...
    const results = {
      optimized: { project: null, org: null },
      unoptimized: { project: null, org: null }
    };

    // 4. BENCHMARK WITH INDEXES
    console.log('\n🧪 Testing with INDEXES (Optimized)...');
    results.optimized.project = await measureQuery(
        'Scenario 1: Project View (Latest 50)', 
        targetProjectId, 
        'SELECT * FROM tasks WHERE project_id = $1 ORDER BY created_at DESC LIMIT 50'
    );
    results.optimized.org = await measureQuery(
        'Scenario 2: Org View (Latest 50)', 
        targetOrgId,
        `SELECT t.* 
         FROM tasks t 
    ...
    // 5. REMOVE INDEXES
    console.log('\n🗑️  Dropping indexes for unoptimized test...');
    const indexesToDrop = [
    ...
    results.unoptimized.project = await measureQuery(
        'Scenario 1: Project View (Latest 50)', 
        targetProjectId, 
        'SELECT * FROM tasks WHERE project_id = $1 ORDER BY created_at DESC LIMIT 50'
    );
    results.unoptimized.org = await measureQuery(
        'Scenario 2: Org View (Latest 50)', 
        targetOrgId,
        `SELECT t.* 
         FROM tasks t 
...
function printReport(results) {
  console.log('\n' + '='.repeat(90));
  console.log('📊 PERFORMANCE BENCHMARK REPORT: 2.75M TASKS (NO PADDING / STANDARD ROW SIZE)');
  console.log('='.repeat(90));
  console.log(`${'Query Scenario'.padEnd(35)} | ${'Index Scan (ms)'.padEnd(15)} | ${'Seq Scan (ms)'.padEnd(15)} | ${'Blocks Saved'.padEnd(12)}`);
  console.log('-'.repeat(90));

  const scenarios = [
      { key: 'project', label: 'Scenario 1: Project View' },
      { key: 'org', label: 'Scenario 2: Org View' }
  ];
